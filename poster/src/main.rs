...
async fn main() {
    ...
    let routes = Router::new()
        ...
        .route(
            "/add_comment_to_comment/:post_id/:comment_id",
            post(views::add_comment_to_comment),
        ) // if logged in
        ...
        .route(
            ...
            get(views::upvote_comment),
        ) // if logged in
        .route(
            "/downvote_comment/:post_id/:comment_id",
            get(views::downvote_comment),
        ) // if logged in
...
async fn create_tables(pool: &SqlitePool) {
    sqlx::query(&utils::read_file(
        &(SQL_PATH.to_string() + "makeTables.sql"),
    ))
    .execute(pool)
    .await
    .unwrap();
